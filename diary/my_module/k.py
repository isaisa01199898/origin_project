...
import dlib
import numpy as np
left_pupil_x, left_pupil_y, right_pupil_x, right_pupil_y, sensor_data= None, None, None, None, 0
i=3
...
# 別スレッドで実行し続ける関数
def capture_loop():
    global left_pupil_x, left_pupil_y, right_pupil_x, right_pupil_y, sensor_data
    while True:
        def eye_do():
        ...
        while True:
                ...
                time.sleep(0.1)  # 適当な間隔

# アプリ起動時にスレッド起動
import threading
threading.Thread(target=capture_loop, daemon=True).start()

# data_getは最新のグローバル変数を返すだけ
def data_get():
    return right_pupil_x, right_pupil_y, left_pupil_x, left_pupil_y, sensor_data
